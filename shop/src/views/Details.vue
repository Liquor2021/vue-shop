...
export default {
  ...
  created() {
    window.scrollTo(0, 0);
    if (this.msg.id) {
      if (this.msg.id >= 120 && this.msg.id < 130) {
        console.log(this.msg.image);
        this.banner.push(this.msg.image);
      } else {
        this.get(this.msg.id);
      }
    } else {
      let a = JSON.parse(localStorage.getItem("msg")).id;
      if (a >= 120 && a < 130) {
        this.banner.push(JSON.parse(localStorage.getItem("msg")).image);
      } else {
        this.get(JSON.parse(localStorage.getItem("msg")).id);
      }
    }
  },
  components: {
